...
var VirtualizedScrollBar = function (_Component) {
	...
	function VirtualizedScrollBar(props) {
		...
		var _this = possibleConstructorReturn(this, (VirtualizedScrollBar.__proto__ || Object.getPrototypeOf(VirtualizedScrollBar)).call(this, props));
	...
	createClass(VirtualizedScrollBar, [{
		...
		value: function getListToRender(list) {
			...
			if (this.state.scrollOffset === 0 && this.props.stickyElems && this.props.stickyElems.length === 0) {
				...
				var _loop = function _loop(index) {
					...
					if (_this2.props.stickyElems.find(function (id) {
			...
			var start = Math.max(0, (smallestIndexVisible != null ? smallestIndexVisible : 0) - this.state.elemOverScan);
		...
		value: function render() {
			...
			var childrenWithProps = React.Children.map(children, function (child, index) {
				return React.cloneElement(child, { originalindex: index });
			});
			...
			var unrenderedBelow = hasScrolled ? (listToRender && listToRender.length > 0 ? listToRender[0].props.originalindex : 0) - (this.stickyElems ? this.stickyElems.length : 0) : 0;
			var unrenderedAbove = listToRender && listToRender.length > 0 ? childrenWithProps.length - (listToRender[listToRender.length - 1].props.originalindex + 1) : 0;
			var belowSpacerStyle = this.props.disableVirtualization ? { width: '100%', height: 0 } : { width: '100%', height: unrenderedBelow ? unrenderedBelow * elemHeight : 0 };

			var aboveSpacerStyle = this.props.disableVirtualization ? { width: '100%', height: 0 } : { width: '100%', height: unrenderedAbove ? unrenderedAbove * elemHeight : 0 };
			...
			return React.createElement(
				...
				React.createElement(
					'div',
					{ className: 'virtualized-scrollbar-inner', style: innerStyle, ref: function ref(div) {
							return _this3._test = div;
						} },
					React.createElement('div', { style: belowSpacerStyle, className: 'below-spacer' }),
					listToRender,
					React.createElement('div', { style: aboveSpacerStyle, className: 'above-spacer' })
				)
			);
		}
	}]);
...
var DynamicVirtualizedScrollbar = function (_Component) {
	...
	function DynamicVirtualizedScrollbar(props) {
		...
		var _this = possibleConstructorReturn(this, (DynamicVirtualizedScrollbar.__proto__ || Object.getPrototypeOf(DynamicVirtualizedScrollbar)).call(this, props));
		...
		_this.virtualizationThreshold = props.virtualizationThreshold != null ? props.virtualizationThreshold : 40;
		...
		_this.state = {
			...
			belowSpacerHeight: initialElemsToRender === _this.props.listLength - 1 ? 0 : (Math.floor(props.listLength * 0.75) - 1) * props.minElemHeight,
			numElemsSized: 0,
			totalElemsSizedSize: 0,
			renderPart: null
		};
		_this.elemOverScan = _this.props.elemOverScan != null ? _this.props.elemOverScan : _this.props.simplified ? 0 : 10;
		_this.childRefs = [];
		_this.stickyElems = null;
	...
	createClass(DynamicVirtualizedScrollbar, [{
		...
		value: function componentDidMount() {
			...
			if (this.itemsContainer && this.itemsContainer.children && !this.props.simplified) {
				...
				var lastElemBounds = lastElem && lastElem.firstElementChild ? lastElem.firstElementChild.getBoundingClientRect() : {};
		...
		value: function shouldComponentUpdate(nextProps, nextState) {
			...
			return this.props.stickyElems && this.props.stickyElems.length > 0 || this.props.listLength !== nextProps.listLength || nextState.aboveSpacerHeight !== this.state.aboveSpacerHeight || nextState.belowSpacerHeight !== this.state.belowSpacerHeight || nextState.firstRenderedItemIndex !== this.state.firstRenderedItemIndex || nextState.lastRenderedItemIndex !== this.state.lastRenderedItemIndex || this.propsDidChange(this.props, nextProps);
		}
	}, {
		...
		value: function propsDidChange(props, nextProps) {
			...
			return newProps.filter(function (_ref) {
				var _ref2 = slicedToArray(_ref, 2),
				    key = _ref2[0],
				    val = _ref2[1];
		...
		value: function updateRemainingSpace() {
			...
			var lastRenderedItemIndex = Math.min(this.props.listLength - 1, this.state.lastRenderedItemIndex + this.elemOverScan);
		...
		value: function getListToRender(list) {
			...
			list.forEach(function (child, index) {
				// Maintain elements that have the alwaysRender flag set. This is used to keep a dragged element rendered, even if its scroll parent would normally unmount it.
				if (_this5.props.stickyElems.find(function (id) {
					return id === child.props.draggableId;
				})) {
		...
		value: function setScrollSection(scrollOffset, isScrollingDown) {
			...
			if (this.state.renderPart !== sectionScrolledTo) {
				...
				var firstIndex = sectionScrolledTo == 0 ? 0 : isScrolledToLastSection ? Math.max(0, this.props.listLength - 1 - elemsPerSection - this.optimisticCount) : sectionScrolledTo * elemsPerSection - this.optimisticCount;
				var lastIndex = Math.min(this.props.listLength - 1, firstIndex + elemsPerSection + this.optimisticCount);
				...
				var didBounce = isScrollingDown ? sectionScrolledTo < this.state.renderPart : sectionScrolledTo > this.state.renderPart;
				if (didBounce) {
					return;
				}
				var aboveSpacerHeight = sectionScrolledTo === 0 ? 0 : sectionScrolledTo * this.props.containerHeight - optimismAboveHeight;
				var belowSpacerHeight = isScrolledToLastSection ? 0 : (numSections - sectionScrolledTo) * this.props.containerHeight - optimismBelowHeight;
				this.setState({
		...
		value: function updateAverageSizing() {
			...
			if (numSized !== this.state.numElemsSized) {
				var scrollHeight = this.scrollBars ? this.scrollBars.getScrollHeight() : window.innerHeight;
		...
		value: function render() {
			...
			var childrenWithProps = React.Children.map(children, function (child, index) {
				return React.cloneElement(child, { originalindex: index, ref: function ref(node) {
			...
			var aboveSpacerStyle = { border: this.props.showIndicators ? 'solid 3px purple' : 'none', width: '100%', height: Math.max(this.state.aboveSpacerHeight - overScanHeightAbove, 0) };
			if (this.stickyElems && this.stickyElems.length > 0) {
			...
			return React.createElement(
				...
				,
				_extends({ onScrollFrame: this.handleScroll.bind(this),
					ref: function ref(div) {
				...
				this.props.showIndicators ? React.createElement('div', { className: 'first-indicator', style: firstIndicatorStyle }) : null,
				this.props.showIndicators ? React.createElement('div', { className: 'last-indicator', style: lastIndicatorStyle }) : null,
				React.createElement(
					'div',
					{ className: 'virtualized-scrollbar-inner', style: innerStyle, ref: function ref(div) {
							return _this8.inner = div;
						} },
					React.createElement('div', { ref: function ref(div) {
							return _this8.aboveSpacer = div;
						}, style: aboveSpacerStyle, className: 'above-spacer' }),
					React.createElement(
						'div',
						{ className: 'list-items', style: listItemsStyle, ref: function ref(div) {
					...
					React.createElement('div', { ref: function ref(div) {
						...
						}, style: belowSpacerStyle, className: 'below-spacer' })
				)
			);
		}
	}]);
...
var Droppable = function (_Component) {
	...
	function Droppable(props) {
		...
		var _this = possibleConstructorReturn(this, (Droppable.__proto__ || Object.getPrototypeOf(Droppable)).call(this, props));
	...
	createClass(Droppable, [{
		...
		value: function pushPlaceholder(children) {
			...
			var placeholderHeight = this.props.dynamicElemHeight ? this.getDraggedElemHeight() : this.props.elemHeight ? this.props.elemHeight : this.defaultElemHeight;
			var style = void 0;
			...
			if (this.state.placeholder) {
				listToRender.forEach(function (elem, index) {
					if (elem && elem.props && elem.props.draggableId === _this3.state.placeholder && !pushedPlaceholder) {
						listToRender.splice(index, 0, React.createElement(
							'div',
							{ key: 'placeholder', draggableid: 'placeholder', className: 'draggable-test', style: style },
							React.createElement('p', { className: 'placeholder-text' })
						));
						pushedPlaceholder = true;
					}
				});
			} else if (!pushedPlaceholder) {
				listToRender.push(React.createElement(
					'div',
					{ key: 'placeholder', draggableid: 'placeholder', className: 'draggable-test', style: style },
					React.createElement('p', { className: 'placeholder-text' })
				));
		...
		value: function render() {
			...
			outerContainerHeight = this.props.enforceContainerMinHeight ? this.props.containerHeight : shouldScroll ? this.props.containerHeight : this.props.containerMinHeight && this.props.containerMinHeight >= calculatedRowMinHeight ? this.props.containerMinHeight : Math.min(calculatedRowMinHeight + listHeaderHeight, this.props.containerHeight);

			var draggedElemId = this.state.currentlyActiveDraggable ? this.state.currentlyActiveDraggable.draggableId : null;
			var CustomTag = this.props.tagName ? this.props.tagName : 'div';
			var headerWithProps = this.props.listHeader != null && this.props.listHeaderHeight != null ? React.cloneElement(this.props.listHeader, {
				draggableid: this.props.droppableId + '-header'
			}) : null;
			var isActive = this.state.droppableActive && this.state.droppableActive === this.props.droppableId;
			var headerActive = isActive && this.state.placeholder && this.state.placeholder.includes('header');

			return React.createElement(
				CustomTag,
				_extends({}, propsObject, { style: { height: outerContainerHeight, minHeight: outerContainerHeight, maxHeight: outerContainerHeight, overflow: 'hidden' } }),
				React.createElement(
					'div',
					{ className: 'header-wrapper ' + (headerActive ? this.props.activeHeaderClass : '') },
					headerWithProps
				),
				this.props.hideList ? null : shouldScroll && !this.props.disableScroll ? this.props.externalVirtualization ? React.createElement(
					lib_1
					// onScrollStop={this.onScrollStop.bind(this)}
					,
					_extends({ onScrollFrame: this.props.onScroll,
						ref: function ref(div) {
				...
				) : this.props.dynamicElemHeight ? React.createElement(
	...
	createClass(DragDropContext, [{
		...
		value: function dispatchPlaceholder() {
			if (this.state.draggedElem && this.state.dragActive && this.state.droppableActive) {
				event_manager_4(this.state.dragAndDropGroup.placeholderEvent, this.state.placeholder, this.state.droppableActive, this.state.draggedElem);
			} else {
		...
		value: function onMoveScroll(x, y, droppable) {
			...
			if (this.state.dragActive && this.state.draggedElem) {
				...
				var scrollThreshold = this.props.autoScrollThreshold || Math.max(50, Math.round(screenWidth * 0.05));
				...
				var shouldScrollGlobally = isNearPageBottom || isNearPageTop || isNearPageLeft || isNearPageRight;
				var canScrollGlobally = this.getCanScrollDirection(isNearPageBottom ? 'down' : isNearPageTop ? 'up' : isNearPageLeft ? 'left' : isNearPageRight ? 'right' : '');
		...
		value: function onDragMove(draggable, droppable, draggableHoveredOverId, x, y, sectionId) {
			if (draggable && droppable) {
				var shouldUpdateDraggable = this.state.draggedElem != null ? this.state.draggedElem.id !== draggable.id : draggable != null;
				var shouldUpdateDroppable = this.state.droppableActive != null ? this.state.droppableActive !== droppable : droppable != null;
				var shouldUpdatePlaceholder = this.state.placeholder != null ? this.state.placeholder !== draggableHoveredOverId : draggableHoveredOverId != null;
				var shouldUpdateSectionId = this.state.targetSection != null ? this.state.targetSection !== sectionId : sectionId != null;
				var mutationObject = {};
			...
			if (this.state.dragActive && this.state.draggedElem) {
				if (this.state.globalScroll && (this.state.globalScrollXDirection || this.state.globalScrollYDirection) && this.outerScrollBar) {
					...
					if (this.state.increaseYScroll) {
						event_manager_4(this.state.dragAndDropGroup.scrollEvent, this.state.droppableActive, 15);
					} else {
						event_manager_4(this.state.dragAndDropGroup.scrollEvent, this.state.droppableActive, -15);
					}
					requestAnimationFrame(function () {
		...
		value: function render() {
			...
			return this.props.outerScrollBar ? React.createElement(
				'div',
				{ ref: function ref(div) {
						return _this4.container = div;
					}, className: 'drag-drop-context', style: { display: 'flex', flexDirection: 'column' } },
				React.createElement(
					lib_1,
					_extends({
						onScroll: this.handleScroll.bind(this),
...
var DragScrollBar = function (_Component) {
	...
	function DragScrollBar(props) {
		...
		var _this = possibleConstructorReturn(this, (DragScrollBar.__proto__ || Object.getPrototypeOf(DragScrollBar)).call(this, props));
	...
	createClass(DragScrollBar, [{
		...
		value: function getCanScrollDirection(dir) {
			...
			var containerHeight = this.container ? this.container.clientHeight : this.props.maxHeight ? this.props.maxHeight : window.innerHeight;
...
var ExampleBoard = function (_Component) {
	...
	function ExampleBoard(props) {
		...
		var _this = possibleConstructorReturn(this, (ExampleBoard.__proto__ || Object.getPrototypeOf(ExampleBoard)).call(this, props));
	...
	createClass(ExampleBoard, [{
			...
			list.forEach(function (entry, index) {
				...
				entry.items.forEach(function (item) {
					list.push(React.createElement(
						Draggable,
						{ dragAndDropGroup: _this2.dragAndDropGroupName, draggableId: item.id, dragDisabled: false, key: item.id },
						React.createElement(
							'div',
							{ onClick: function onClick() {
									return alert('A click is not a drag');
								}, className: 'draggable-test', style: { border: 'solid 1px black', height: '48px', backgroundColor: 'white', flexGrow: 1 } },
							React.createElement(
								'p',
		...
		value: function onDragEnd(source, destinationId, placeholderId) {
			...
			var indexToInsert = placeholderId === 'END_OF_LIST' ? listToAddTo.items.length : placeholderId.includes('header') ? 0 : listToAddTo.items.findIndex(function (item) {
		...
		value: function render() {
			...
			var getListHeader = function getListHeader(index) {
				return React.createElement(
					...
					React.createElement(
						'button',
						{ className: 'scroll-button', onClick: function onClick() {
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'div',
							{ className: 'controls' },
							React.createElement('div', { title: 'Sidescroll Backwards', className: 'backwards', onClick: this.sideScroll.bind(this, -50) }),
							React.createElement('div', { itle: 'Sidescroll Forwards', className: 'forwards', onClick: this.sideScroll.bind(this, 50) })
						),
						React.createElement(
							'div',
							...
							React.createElement(
								'button',
								{ className: 'row-split-button first-button' + (!this.state.split ? ' active' : ''), onClick: this.toggleSplit.bind(this) },
								'Single Row'
							),
							React.createElement(
								'button',
								{ className: 'row-split-button second-button' + (this.state.split ? ' active' : ''), onClick: this.toggleSplit.bind(this) },
								'Multi Row'
							)
						)
					),
					...
					React.createElement(
						'div',
						{ className: 'test-container', style: { display: 'flex', flexDirection: 'row', position: 'relative' } },
						elemsToRender.map(function (elem, index) {
							return !_this3.state.split || index < elemsToRender.length / 2 ? React.createElement(
								'div',
								{ className: 'sizer', style: { flexGrow: 1, minWidth: 350 }, key: index + elem.droppableId },
								React.createElement(
									Droppable
					...
					this.state.split ? React.createElement(
						'div',
						{ className: 'test-container', style: { display: 'flex', flexDirection: 'row', position: 'relative' } },
						elemsToRender.map(function (elem, index) {
							return index >= elemsToRender.length / 2 ? React.createElement(
								'div',
								{ className: 'sizer', style: { flexGrow: 1, minWidth: 350 }, key: index + elem.droppableId },
								React.createElement(
									Droppable,
...
var DynamicHeightExample = function (_Component) {
	...
	function DynamicHeightExample(props) {
		...
		var _this = possibleConstructorReturn(this, (DynamicHeightExample.__proto__ || Object.getPrototypeOf(DynamicHeightExample)).call(this, props));
	...
	createClass(DynamicHeightExample, [{
		...
		value: function generateTestList(num, numItems) {
			...
			var pseudoRandomSize = function pseudoRandomSize(i) {
				return 50 + ((i + 1) * (num + 1) % 5 === 0 ? 200 : (i + 1) * (num + 1) % 4 === 0 ? 150 : (i + 1) * (num + 1) % 3 === 0 ? 100 : (i + 1) * (num + 1) % 2 === 0 ? 50 : 0);
			};
			var sectionId = 0;
			for (var i = 0; i < numItems; i++) {
				if (i % 3 === 0) {
		...
		value: function getElemsToRender(list) {
			...
			list.forEach(function (entry, index) {
				...
				entry.items.forEach(function (item, idx) {
					if (_this2.state.useSections && !seenSections.includes(entry.index + '-' + item.sectionId)) {
						list.push(React.createElement(
							Draggable,
							{
							...
							React.createElement(
								'div',
								{ className: 'draggable-test section', style: { height: 50, outline: 'none', backgroundColor: '#dbdbdb', flexGrow: 1 } },
								React.createElement(
									'div',
									{ style: { marginLeft: '5px', paddingTop: '8px' }, className: 'item-name row' },
									React.createElement('div', { className: 'person-image' }),
									item.sectionId
								)
							)
						));
					...
					list.push(React.createElement(
						Draggable,
						{ sectionId: item.sectionId, dragAndDropGroup: _this2.dragAndDropGroupName, draggableId: item.id, dragDisabled: false, key: item.id },
						React.createElement(
							'div',
							{
								onClick: function onClick() {
								...
								className: 'draggable-test' + (_this2.state.recentlyMovedItem === item.id ? ' dropGlow' : ''),
		...
		value: function onDragEnd(source, destinationId, placeholderId, sectionId) {
			...
			var indexToInsert = placeholderId != null ? placeholderId === 'END_OF_LIST' ? listToAddTo.items.length : placeholderId.includes('header') ? 0 : listToAddTo.items.findIndex(function (item) {
				return item.id === placeholderId;
			}) : sectionId != null ? listToAddTo.items.findIndex(function (item) {
			...
			var isSameSection = targetElem && targetElem.sectionId && source.sectionId && targetElem.sectionId === source.sectionId;
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							...
							React.createElement(
								'button',
								{ className: 'indicator-button' + (this.state.showIndicators ? ' active' : ''), onClick: this.toggleIndicators.bind(this) },
								'Show Virtualization Indicators'
							),
							this.TEST_ENV ? React.createElement(
								'button',
								{ className: 'indicator-button' + (this.state.useSections ? ' active' : ''), onClick: this.toggleUseSections.bind(this) },
								'Use Sections'
							) : null
						)
					),
					...
					React.createElement(
						...
						React.createElement('input', { type: 'checkbox', value: this.state.lazyLoad, onClick: this.handleLazyLoadChange.bind(this) })
					),
					React.createElement(
						'div',
						{ className: 'test-container', style: { display: 'flex', flexDirection: 'row', position: 'relative' } },
...
var ExampleMultipleDroppables = function (_Component) {
	...
	function ExampleMultipleDroppables(props) {
		...
		var _this = possibleConstructorReturn(this, (ExampleMultipleDroppables.__proto__ || Object.getPrototypeOf(ExampleMultipleDroppables)).call(this, props));
					...
					React.createElement(
						...
						React.createElement(
							...
							elemsToRender.map(function (elem, index) {
								return !_this3.state.split || index < elemsToRender.length / 2 ? React.createElement(
									'div',
									{ className: 'sizer with-space', style: { flexGrow: 1, minWidth: 350 }, key: index + elem.droppableId },
									React.createElement(
										Droppable
										//enforceContainerMinHeight={true}
										,
...
export { Draggable, Droppable, DragDropContext, DragScrollBar, VirtualizedScrollBar, ExampleBoard, DynamicHeightExample, ExampleMultipleDroppables };
