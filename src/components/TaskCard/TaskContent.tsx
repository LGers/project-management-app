...
export const TaskContent = (props: ITaskProp) => {
  const { open, onClose, item, openDelDialog, onUpdateTask } = props;

  const [oldTaskTitle, setOldTaskTitle] = useState<string>(item.task.title);
  const [newTaskTitle, setNewTaskTitle] = useState<string>(item.task.title);
  const [focusTitle, setFocusTitle] = useState(false);

  const [isDescriptionBtnVisible, setIsDescriptionBtnVisible] = useState<boolean>(false);
  const [oldTaskDescription, setOldTaskDescription] = useState<string>(item.task.description);
  const [newTaskDescription, setNewTaskDescription] = useState<string>(item.task.description);
